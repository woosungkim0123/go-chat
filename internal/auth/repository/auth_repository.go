...
func (r *AuthRepository) FindUserByLoginID(LoginID string) (*domain.User, *apperror.CustomError) {
	...
	dbError := r.db.View(func(tx *bbolt.Tx) error {
		...
		if b == nil {
			...
			return fmt.Errorf(string(apperror.NotFoundBucket))
		}
		...
		foundUserError := b.ForEach(func(k, v []byte) error {
			...
			if jsonError := json.Unmarshal(v, &tempUser); jsonError != nil {
				...
				return fmt.Errorf(string(apperror.FailJsonUnmarshal))
			}
		...
		if foundUserError == nil {
			...
			return fmt.Errorf(string(apperror.NotFoundUserByLoginID))
	...
	if dbError != nil {
		if dbError.Error() == string(apperror.NotFoundUserByLoginID) {
			return nil, &apperror.CustomError{Code: apperror.NotFoundUserByLoginID, Message: "LoginID에 해당하는 유저를 찾을 수 없습니다."}
		}
...
func (r *AuthRepository) FindByID(userID int) (*domain.User, *apperror.CustomError) {
	var user *domain.User
	...
	dbError := r.db.View(func(tx *bbolt.Tx) error {
		...
		if userData == nil {
			...
			return fmt.Errorf(string(apperror.NotFoundUserByID))
		}
	...
	if dbError != nil {
		// 에러 처리 로직
		if dbError.Error() == string(apperror.NotFoundUserByID) {
			return nil, &apperror.CustomError{Code: apperror.NotFoundUserByID, Message: "ID에 해당하는 유저를 찾을 수 없습니다."}
		}
...
func (r *AuthRepository) GetUserListWithoutSelf(userID int) ([]domain.User, *apperror.CustomError) {
